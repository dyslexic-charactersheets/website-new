...
import { SkillsProfile, getSkillsProfile } from './cls/SkillsProfile.js';
import { has, isEmpty } from './util.js';
// import { drawText } from 'pdf-lib';


const skillFontSize = 8;
const attrFontSize = 10.4;

export async function writeSkills(doc, pageInfo) {
  let skills = doc.gameData.getSkills(pageInfo, doc.settings, doc.classes);
  if (skills.length == 0) {
    return;
  }
      ...
      if (doc.settings.classes) {
        ...
        if (!isSubskill && !skill.noRanks) {
          if (doc.settings.isPathfinder || doc.settings.isStarfinder) {
            ...
            for (let i = 0; i < n; i++) {
              let cls = doc.classes.length >= i ? doc.classes[i] : null;
  ...
  for (let skill of topSkills) {
    ...
    if (has(subskillsBySkill, skill.name)) {
      let subSkills = sortSkills(subskillsBySkill[skill.name]);
